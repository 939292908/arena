...
const logic = {
    ...
    setMaxTransfer () {
        ...
        // 设置 我的钱包最大划转
        this.setMyWalletMaxTransfer();
        // console.log(9999, wlt.wallet, this.maxMoney);
        // m.redraw();
    },
    // 设置 我的钱包最大划转
    setMyWalletMaxTransfer () {
        if (wlt.wallet) { // 所有钱包 和 从xx钱包id 都存在
            const wallet = wlt.wallet["03"]; // 对应钱包
            for (const item of wallet) {
                if (item.coin === this.coin) { // 找到对应币种
                    this.myWalletMax = utils.toFixedForFloor(item.wdrawable || 0, 4); // 设置最大可以金额
                }
            }
        } else {
            this.myWalletMax = 0;
